...
def process_gates_shadow(payload: dict, trace_id: str) -> dict:
    ...
    # -- Full Processing (Post-Cutover Apr 1, 2026) --
    # All contacts that pass gates get processed. Canary mode retired.
    from app.services.canary_router import track_canary_result

    if result.overall_decision == GateDecision.PASS:
        ...
        logger.info(
            "routed_to_processing",
            trace_id=trace_id,
            contact_id=result.contact_id,
        )
    else:
        # Blocked by gates -- log only
        logger.info(
            "gate_blocked",
            trace_id=trace_id,
            contact_id=result.contact_id,
